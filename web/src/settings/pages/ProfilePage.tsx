...
import type { PluginSettingsForm } from './SettingsPage';
import { SecretField, type SecretMeta } from '../../shared/views/SecretField';
import { RepoListView } from '../../projects/pages/ProjectListPage';
import { RepoDetailOverlay } from '../../projects/pages/ProjectDetailPage';
...
export const ProfileOverlay: React.FC<ProfileOverlayProps> = ({
  ...
  const [tab, setTab] = useState<'settings' | 'repos'>('settings');
  const [selectedGroup, setSelectedGroup] = useState<RepoGroup | null>(null);

  if (selectedGroup) {
    return (
      <RepoDetailOverlay group={selectedGroup} onBack={() => setSelectedGroup(null)} />
    );
  }
  ...
  return (
    <section className="page-overlay settings-overlay-page">
      ...
      <div className="overlay-body settings-overlay-body">
        {tab === 'repos' ? (
          <RepoListView onSelectGroup={setSelectedGroup} />
        ) : (
          <>
      ...
      {tab === 'settings' && (
        <footer className="overlay-footer">
          <button
            type="button"
            className="btn"
            onClick={() => onSave(apiKeyEdit, emailPasswordEdit)}
          >
            保存
          </button>
        </footer>
