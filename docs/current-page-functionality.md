...
- 顶部独立展示「仓库日报」：按日期倒序列出关联当前项目的 `completed`
  内容（日期 - 日志列表）；
- 「生成工作日志」按每个选中日期分别调用 AI 润色后生成内容；
- 生成内容写入对应日期 JSON 的 `completed`，同时写入指向当前项目的
  `projectLinks`；
- 未配置 AI API Key 或 AI 润色失败时阻断写入，不回退原始 Commit 文本；
- 重复生成时对内容和项目关联去重；
- 成功日期取消选择并重新加载详情回显，失败日期保留选择并显示原因；
- Commit 活动区仅展示 Commit 展开列表，不再重复显示工作日志条目。
